...
iris %>% dplyr::summarise(average_Pepal.Width = mean(Petal.Width))
```
...
```{r practice_3, exercise = tutorialize}
iris %>% dplyr::distinct(Species,Petal.Length) %>%
...
```{r practice_4,exercise = tutorialize}
iris %>%
  dplyr::mutate(color = case_when(
    Species == "setosa" ~ "blue",
    Species == 'versicolor' ~ "red",
