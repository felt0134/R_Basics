...
```{r problem_3, exercise=TRUE,exercise.setup="problem_1"}
...
Rember 0.75 is a vector of length 1
...
### Q4: Create a new vector using the `seq()` function which ranges from 10 to 80 and increases by values of 10. Assign this new vector the name "vector_2". Then, on the next line use the `plot()` function to plot vector_1 (created in the previous problem) on the x axis and vector_2 on the y axis.

```{r problem_4, exercise=TRUE,exercise.setup="problem_3"}
...
```{r problem_12, exercise=TRUE,exercise.setup="problem_11"}
...
The answer to question 16 is now an argument in the `mean()` function
...
### Q18: Repeat question 7 but "wrap" or "nest" the `mean()` function as an argument *inside* the `round()` function so to round the output to two decimal places.
...
```{r problem_21, exercise=TRUE,exercise.setup="problem_19"}
...
```{r problem_23, exercise=TRUE,exercise.setup = "problem_22"}
...
The column should be the sole argument in the function
