...
output: 
  learnr::tutorial:
    progressive: true
tutorial:
  id: "Week_4_Functions_Data_Types"
...
## Vectors, not just numbers

As a reminder, vectors form the building blocks of storing data and are an essential concept towards understanding `tidy data` principles. A `vector` is a sequence of `elements`, composed of the same `data type`, of a given `length`.

`length` is the number of elements (e.g., distinct values) in the sequence. The vector 1,2,3 has length = 3 because it has three elements in the sequence, and the vector 1,2,4,8 has length = 4 because it has 4 elements in the sequence.
...
R is a programming language that relies on using `functions`. Almost everything you do in R will require, to some extent, the use of functions. Therefore, functions are really the workhorse of data wrangling and analysis in R; they can import datasets, crunch numbers, filter data, combine datasets together, create diverse data visualizations ranging from simple x-y plots to complex maps, or run statistical models, just to name a few. Eventually, you will even be able to write your own customized functions.

A function has three basic visual characteristics:

* A function has a name that is descriptive of what it does.
* A function has bounded parentheses to the right of the name that contain the inputs to be "passed" into the function. This is in contrast to operators, where whatever is being "operated on" can be on either side of the symbol - case in point the `:` operator to create a sequence of values (AKA a vector).
* A function has `arguments` that defines the inputs to be passed into the function, which are needed for the function to *return* an output. All arguments are contained in the bounded parentheses. Once you have the function defined and the arguments provided, you can *call* (execute or run) the function to return an output. Functions are useful because they work as shorthands for longer blocks of code to execute specific tasks.

One important function that creates vectors is the `c()` function. This function "**c**ombines"  multiple values to form a single vector. This allows you to create all sorts of vectors. Run the code below to create an (atomic) character vector:
...
Notice how each value passed into the `c()` function is separated by a comma. Commas separate each argument in a function, and here each value is treated as an argument. An argument is a value or variable that you "pass" into a function. Argument(s) direct the inputs to functions, who then return a desired output(s). Above, the arguments passed into the `c()` function are different characters, and the output *returned* is a character vector of length 4.

### recycling revisted

Adding two vectors together highlights an important behavior of data (and specifically vectors) in R called `recycling`. If we added two vectors together that had different lengths, what do you think might happen? First let's add two vectors of the same length together. Run the code below:
...
*Coercion: Another behavior of atomic vectors*

You can also use `c()` to combine different types of atomic vectors (e.g., integers and character data types). The dilemma is than an atomic vector can only be composed of a single data type. The result is that R will "coerce" the resulting vector to be composed of the most flexible data type. If you combine an integer and a double vector together, it will become a double vector because real numbers have more flexibility in what they can be than integers (e.g., 1 can be 1.0, but you must round to turn 1.5 into an integer!). The same idea applies to combining a double with a character vector. Characters can basically take on any values, be it a number or letters, but you cannot convert a letter to a double. This results in a "coercion hierarchy" from least to most flexible data type: `NA -> integer -> double -> character`. View and code along in the associated guided practice video for a demonstration of this.
...
learnr:: quiz(
  ...
  question("If you combine an integer vector with a character vector, what type of vector will be returned?",
    answer("integer"),
    answer("double"),
    answer("character",correct = TRUE),
    answer("Nothing: this is not possible because everything must be the same data type")
  )
