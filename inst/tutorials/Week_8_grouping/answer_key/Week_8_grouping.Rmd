...
```{r practice_1, exercise = tutorialize}
...
```{r practice_2, exercise = tutorialize}
iris %>% dplyr::summarise(min_Petal.Width = min(Petal.Width))
```
...
```{r practice_3, exercise = tutorialize,exercise.setup = demo_4}
dplyr::summarise(iris,min_Petal.Width = min(Petal.Width),
                 max_Petal.Width = max(Petal.Width),
...
```{r practice_4, exercise = tutorialize,exercise.setup=demo_4}
iris %>% group_by(Species) %>%
...
```{r practice_5, exercise=tutorialize}
library(lterdatasampler)
```
...
```{r practice_6, exercise=tutorialize}
...
```{r practice_7, exercise=tutorialize}
...
```{r practice_8, exercise=tutorialize}
...
```{r practice_9, exercise=tutorialize}
knz_bison %>% dplyr::mutate(age = rec_year - animal_yob) %>%
...
```{r practice_10, exercise=tutorialize}
knz_bison %>% dplyr::mutate(age = rec_year - animal_yob) %>%
  dplyr::group_by(rec_year,animal_sex) %>%
  dplyr::summarise(average_age = mean(age)) %>%
...
```{r practice_11, exercise=tutorialize}
knz_bison %>% dplyr::mutate(age = rec_year - animal_yob) %>%
  dplyr::group_by(rec_year,animal_sex) %>%
  dplyr::summarise(average_age = mean(age)) %>%
